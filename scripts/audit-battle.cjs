...
function simulate(encounter,element,rank,special='burst',smart=true){
  ...
  if(encounter.boss)dragon.training.technique='precision';
  const reserves=encounter.boss?['water','nature'].map((el,i)=>({...dragon,element:el,training:{...dragon.training,id:i+2,name:'Reserve '+i}})):[];
  const b=B.create(dragon,encounter.id,reserves);
  while(['active','switch'].includes(b.phase)){
    ...
    const move=!smart?'strike':intent.power>1?'guard':special==='mend'&&p.hp<p.maxHp*.72&&B.legal(b,'special')?'special':intent.guard&&!encounter.boss?'strike':special==='burst'&&B.legal(b,'special')?'special':B.legal(b,'element')?'element':'strike';
    B.resolve(b,move);
  }
